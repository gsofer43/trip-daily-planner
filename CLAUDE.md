...
The packing page also has a **🌤️ עדכון מזג אוויר** button that fetches a real forecast from Open-Meteo (`fetchDayWeather()`), using a day's "anchor" station (`getDayAnchorStop()`, the same helper the overview map uses) and a calendar date derived from `day.date` ("DD/MM") plus the year parsed out of `tripSubtitle` (`getDayIsoDate()`/`getTripYear()`). Because Open-Meteo's free tier only forecasts ~16 days out, days further away don't hit the forecast endpoint at all — instead `fetchDayWeather()` calls `fetchClimateAverage()`, which pulls a **multi-year historical average** for that calendar date (±`WEATHER_CLIMATE_WINDOW_DAYS`) from the Open-Meteo **Archive** API (ERA5) over the `WEATHER_CLIMATE_YEARS` years before the trip year — one request per location, date-window filtering done client-side. It returns `{ status: 'climate', tempMax, tempMin, precipProb (share of historical days with ≥`WEATHER_CLIMATE_RAIN_MM` mm), sampleYears }`, rendered in the muted style and explicitly labelled "ממוצע רב-שנתי … לא תחזית". A real forecast automatically replaces it once the date enters the 16-day window and the user refreshes. Only if that archive call throws does the day fall back to the old `too-far` "not available yet" message. The clothing note per day is plain if/else on the returned numbers (`buildClothingNote()`, shared by `ok` and `climate`) — nothing inferred beyond what the API returned. `climate` rows count as a real update for `computeLastWeatherUpdateText()`/`successCount`, same as `ok`. Unlike `packingList`, the weather cache (`weatherCache`, keyed by day id with a `fetchedAt` timestamp) lives in its **own** localStorage key (`travelPlannerWeatherCache`), not in `state.data` — it's treated as ephemeral/derived data the user wouldn't want bundled into their trip backup JSON. Page load only reads this cache (`renderWeatherList()`); a network fetch happens solely on button click (`refreshAllWeather()`), and per-day fetch failures degrade to an inline `weather-note-error` message rather than breaking the section.
...
- **Agoda** — serves an empty webpack shell to `fetch` (the hotel name appears zero times in the HTML). Only the **hotel detail page** works: free-text search bounces to the homepage, and `/search?city=` needs Agoda-internal numeric ids we do not have. Dates go in as `checkIn` + `los` (nights), not `checkIn`/`checkOut`. **Sold-out is detected by page TEXT, and is checked before anything else.** Two traps are baked into that sentence, both of which produced real wrong answers before being fixed: (1) the obvious-looking `[data-selenium*="soldout"]` / `[class*="SoldOut"]` selectors match **nothing** on a real sold-out page, so a selector-based check can never return `unavailable` and just waits until it times out; (2) Agoda keeps rendering other things with prices on a sold-out page (a "similar properties" carousel), so counting price tokens anywhere in the body reads a sold-out hotel as **available** — a false alert telling someone to go book a room that is not there. So: sold-out text wins outright, and `available` requires both the room-grid element to exist *and* at least one price. The check also waits for the page to settle first, because Agoda's loading state is otherwise indistinguishable from a genuine zero result.
